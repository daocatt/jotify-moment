  ...
  try {
    ...
    if (urlMatch) {
      ...
      if (parsed && isValidEmbedId(parsed.embedType, parsed.embedId)) {
        ...
        // Plain URL — store as link embed for preview card rendering.
        embedType = "link";
        embedId = rawUrl;
      }
    }
    ...
    if (embedType && embedId) {
      void (async () => {
        try {
          ...
          try {
            let meta: { thumbnailUrl?: string; title?: string; description?: string } | null = null;
 ...
 */
async function fetchLinkOgMeta(
  url: string,
  signal?: AbortSignal,
